...
export default function MedicalRecordsPage() {
  ...
  const fetchMedicalRecords = async (): Promise<void> => {
    ...
    try {
      setIsLoading(true);
      const response = await fetch(`/api/medical-records?patientEmail=${encodeURIComponent(user.email)}`);
                ...
                medicalInfo.map((info) => (
                  <Card key={info.id}>
                    <CardHeader className="flex flex-row items-center justify-between">
                      <CardTitle className="text-lg">{(info as any).title}</CardTitle>
                      <Button
                        variant="outline"
                        size="sm"
                  ...
                  <Card key={allergy.id}>
                    <CardHeader className="flex flex-row items-center justify-between">
                      <div className="flex items-center gap-3">
                        <CardTitle className="text-lg">{(allergy as any).allergen}</CardTitle>
                        <Badge variant={
              ...
              {isLoading ? (
                ...
                medications.map((medication) => (
                  <Card key={medication.id}>
                    <CardHeader className="flex flex-row items-center justify-between">
                      <CardTitle className="text-lg">{(medication as any).medicationName}</CardTitle>
                      <Button
                        variant="outline"
                        size="sm"
              ...
              <div className="space-y-4">
                ...
                {correctionFormData.field && (
                  <>
                    ...
                    <div>
                      <Label htmlFor="suggestedValue">Suggested Correction</Label>
                      <Input
                        id="suggestedValue"
