...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (email: string, password: string, first_name: string, last_name: string, phone?: string): Promise<{ success: boolean; error?: string; requiresOTP?: boolean; contactMethod?: 'phone' | 'email'; contactValue?: string; emailValidationRequired?: boolean }> => {
    ...
    try {
      ...
      // If email validation is required, return success with email validation requirement
      if (data.emailValidationRequired) {
        console.log('Email validation required for sign-up');
        return { 
          success: true, 
      ...
      // If OTP is required (fallback), return success with OTP requirement
      if (data.requiresOTP) {
      ...
      // Immediate signup (shouldn't happen in new flow)
      setUser(data.user);
      localStorage.setItem('dcms_user', JSON.stringify(data.user));
      return { success: true };
    } catch (error) {
